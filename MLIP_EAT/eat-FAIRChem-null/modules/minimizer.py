...
class minimizer:
    """
    minimizer class implements optimization algorithms for minimizing functions using projected limited memory BFGS method.
    ...
    def __init__(self, fun_jac, x0, args, project, proj_args, callback=None, BFGS_hist=50, maxit=100, tol=1e-10, max_line_search=3):
        """
        ...
        while k <= self.maxit:
            if reset == True:
                ...
                if isinstance(f, torch.Tensor):
                    f = f.item()
                alpha = 1e-4  # initial step size for gradient descent (TODO: optimize this automatically instead of hardcoding)
                x = x - alpha * g
                ...
                if SPG_converged == False:
                    ...
                    alpha, f_temp, g_temp = self._line_min(
                        ...
                        maxiter=self.max_line_search,
                    )
                    if (alpha is None) or (alpha < 0) or isinstance(alpha, complex):        
                        # Line search failed. Try resetting quasi-Hessian.
                        reset = True
                        continue
            ...
            if self.callback is not None:
                err_new = torch.norm(self.project(x - g_new, *self.proj_args) - x).item()
                self.callback(x, f_new, err_new, k, converged, *self.args)
            k += 1
    ...
    def _SPG(self, x0, f0, g0, delta, N, M_inv, SPG_hist, maxit=50, tol=1e-3):
        """
        ...
        for k in range(maxit):
            ...
            # print(f"                SPG iteration {k+1}: fun = {f_new_val:.6e}, |P(x-g)-x| = {err:.6e}, a = {a:.6e}, a_bb = {a_bb:.6e}.")
            if err < tol:
                return x_new, True
    ...
    def _line_min(self, fun_jac, x0, f0, g0, p, f_b, args, amax=1, c1=0.0001, maxiter=5):
        """
        ...
        while i <= maxiter:
            ...
            try:
                ...
                g_list.append(g_list[i-1])
                a_list.append(a_list[i] / 2.0)  # just do back tracking if failure occurs
                i += 1
                continue
