...
class objfun:
    ...
    def __init__(
        ...
        self.projector = projector(self.n_mix, self.n_elements, True, self.mu, None, None)
    ...
    def fun_jac(self, z_flat: torch.Tensor):
        ...
        if self.syn_type == "compacting":
            # Compute entropic cieling
            if self.Tsallis_q == 1.0:
                entropy_cieling = self.n_mix * np.log(self.n_elements)
            else:
                entropy_cieling = self.n_mix / (self.Tsallis_q - 1) * (1 - self.n_elements**(1 - self.Tsallis_q))
            entropy_cost, d_entropy_cost_d_entropy_total = softplus(
                entropy_total - entropy_cieling * self.tau, self.gamma
            )
            grad_entropy_cost = d_entropy_cost_d_entropy_total * grad_entropy_flat
        elif self.syn_type == "forcing":
        ...
        # -------------------------
        penalty_coeff = 1.0
        penalty = penalty_coeff * self.n_atoms * 1/2 * torch.sum((comp_frac - self.mu)**2)
        grad_penalty = torch.zeros((self.n_mix, self.n_elements), device=self.device, dtype=Xmix.dtype)
        for i in range(self.n_elements):
            grad_penalty[:, i] = penalty_coeff * (comp_frac[i] - self.mu[i].to(device=self.device, dtype=Xmix.dtype))
        ...
        with torch.no_grad():
            ...
            # discreteness distance: ||Xmix - onehot(argmax(Xmix))||_F
            t = torch.zeros_like(Xmix)
            amax = torch.argmax(Xmix, dim=1)
            t.scatter_(1, amax.unsqueeze(1), 1.0)
            discreteness_dist = float(torch.sqrt(torch.sum((Xmix - t) ** 2)).item())
            ...
            self.last_metrics["eat_weights"] = eat_fairchem.detach().cpu().tolist()
            self.last_metrics["eat_weights_selected"] = Xmix.detach().cpu().tolist()
    ...
    def pbfgs_callback(self, z, f, err, k, converged, *args):
        ...
        # save only accepted PBFGS step metrics
        ...
        self.history["discreteness_dist"].append(self.last_metrics["discreteness_dist"] if self.last_metrics["discreteness_dist"] is not None else np.nan)
        ...
        self.log.info(
            ...
            f"ent/atom={ent:.3e}, PG-error={err:.3e}, comp1={self.z_last[:self.n_x].view(self.n_mix, self.n_elements).mean(dim=0)[0]:.3f}"
        )
