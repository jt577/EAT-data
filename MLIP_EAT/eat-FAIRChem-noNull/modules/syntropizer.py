...
class OptimizationResult:
    ...
    all_history: Dict[str, List[float]]
...
class syntropizer:
    def __init__(
        ...
        nIterations_tol=5,
        tau_init=1.0,
        tau_mult=0.9, # don't make this too small (0.9 or so ensure entropy does not drop too fast)
        gamma=1.0, # don't make this too big (1 is good), otherwise the entropy will drop too fast and optimum will be poor
        Tsallis_q=2.0,
        syn_type="compacting",
        ...
        self.fix = fix
        self.alpha = alpha # dirichlet distribution concentration parameter (one is uniform over simplexes)
        ...
        # projector
        self.projector = projector(self.n_mix, self.n_elements, self.fix, self.mu, self.uc_lower, self.uc_upper)

        # global history across all anneals
        self.all_history: Dict[str, List] = {}


    def _init_z(self):
        dist = torch.distributions.Dirichlet(self.alpha * torch.ones(self.n_elements, dtype=torch.float32, device=self.device))
        x0 = dist.sample((self.n_mix,)).view(-1)
        if self.fix:
            self.z = x0
        else:
    ...
    def _syntropize_phase(self):
        ...
        for anneal in range(self.n_anneals):
            ...
            # append per-anneal history into global history
    ...
    def _relax_and_finalize(self) -> OptimizationResult:
        ...
        # Check if all elements of Xf_mix are either 0 or 1
        if not torch.all((Xf_mix == 0.0) | (Xf_mix == 1.0)):
            self.log.warning("Final mix site compositions are not fully discrete (0 or 1).")
            print("Final mix site compositions are not fully discrete (0 or 1).")
