...
def _as_mapping(section: Any, *, name: str) -> dict[str, Any]:
    """
    Accept either:
      - a dict, e.g. {"alpha": 1.0}
      - a list of dicts, e.g. [{"alpha": 1.0}, {"seed": 0}]
    ...
    if isinstance(section, list):
        ...
        for i, item in enumerate(section):
            ...
            out.update(item)  # later entries override earlier ones
        return out
    raise ValueError(f"config: {name} must be a dict or list[dict], got {type(section)}")


def _get_required(d: dict[str, Any], key: str, *, where: str) -> Any:
    if key not in d:
...
def load_inputs(infile: Path) -> RunInputs:
    ...
    syn_type  = str(syn_cfg.get("type", "compacting"))
    tau_mult  = float(syn_cfg.get("tau_mult", 0.9))
    tau_init = float(syn_cfg.get("tau_init", 1.0))
    gamma     = float(syn_cfg.get("gamma", 1.0))

    if syn_type not in ("forcing", "compacting"):
        raise ValueError("config: syntropization.type must be one of {'forcing','compacting'}")
    ...
    for composition in inp.compositions:
        ...
        for i in range(inp.n_trials):
            ...
            syn = syntropizer(
                ...
                # init + syntropization (from yaml nested sections)
                alpha=inp.alpha,
                Tsallis_q=inp.Tsallis_q,
                syn_type=inp.syn_type,
            ...
            cum_iter += syn_iter # total iterations across all trials
            ...
            with open(f"all_history_i={i}.json", "w") as f:
