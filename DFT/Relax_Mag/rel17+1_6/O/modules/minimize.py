...
def solve_quadratic_form(x0, g, H, N, S, details_file_path, max_iter=1000):
    with open(details_file_path, 'a') as file:
        file.write("Optimizing BFGS quadratic form subproblem...\n")

    # Symmetrize B in case
    H = 1/2*(H + H.T)
    # Make B positive semidefinite in case it is not due to numerical accuracy
    eigenvalues, eigenvectors = np.linalg.eigh(H) # Calculate the eigenvalues and eigenvectors of B
    eigenvalues[eigenvalues <= 0] = 1e-10  # Set any negative eigenvalues to small constant
    H = eigenvectors @ np.diag(eigenvalues) @ eigenvectors.T # Reconstruct the matrix using only non-negative eigenvalues

    I = create_diagonal_vectors(N, S)
    Hx0 = H @ x0
    A = np.arange(N*S)  # Active set, initially all variables
    numit = 1
    converged = False

    while not converged and numit <= max_iter:
        # Build reduced matrices/vectors for active set A
        x0_A = x0[A]
        g_A = g[A]
        H_AA = H[np.ix_(A, A)]
        I_A = I[A, :]
        ...
        # Compute Lagrange multipliers mu
        grad_L = H @ (x - x0) + g + I @ lambd
        mu = grad_L

        # For variables in A, set mu_i = 0 (since x_i > 0)
        mu[A] = 0

        # For variables not in A, mu_i should be >= 0
        not_in_A = np.setdiff1d(np.arange(N*S), A)
        mu_not_in_A = mu[not_in_A]

        # Check dual feasibility
        indices_to_add = not_in_A[mu_not_in_A < -1e-8]

        # For variables in A, check primal feasibility x_i >= 0
        x_A = x[A]
        indices_to_remove = A[x_A < -1e-8]

        # Update active set
        if len(indices_to_add) == 0 and len(indices_to_remove) == 0:
            converged = True
        else:
            # Remove variables where x_i < 0
            if len(indices_to_remove) > 0:
                with open(details_file_path, 'a') as file:
                    file.write(f"    ->Iteration {numit}: Removing {len(indices_to_remove)} variables from active set.\n")
                x[indices_to_remove] = 0
                A = np.setdiff1d(A, indices_to_remove)
            # Add variables where mu_i < 0
            if len(indices_to_add) > 0:
                with open(details_file_path, 'a') as file:
                    file.write(f"    ->Iteration {numit}: Adding {len(indices_to_add)} variables to active set.\n")
    ...
    value = 1/2 * (x - x0).T @ H @ (x - x0) + g.T @ (x - x0)
    with open(details_file_path, 'a') as file:
        file.write(f'    Successfully optimized after {numit-1} iterations. Df (must be negative) = {value}\n')
...
def PBFGS(fun, x0, jac, args, S,  folder, progress_file_path, details_file_path, callback, B=None, project=project_to_simplex_fast, maxit=10, tol=1e-3):
    ...
    while k <= maxit:
        ...
        if alpha is None:
            with open(details_file_path, 'a') as file:
                file.write(f"Since line search failed, resetting Hessian to the identity and performing gradient descent step with alpha = 1e-2.\n")
            x_star = solve_quadratic_form(x0=x, g=g, H=B, N=N, S=S, details_file_path=details_file_path)
            x_star = project(x_star, N) # project result to be safe
            d = x_star - x
            B = 5 *  np.linalg.norm(g) * I # Reset quasi-Hessian
            alpha = 1e-4
            x += alpha * d
            x = project(x, N)
            f_new = fun(x, unique_folder, *args)
            g_new = jac(x, unique_folder, *args)
