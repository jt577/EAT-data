...
elements = ['Ni', 'Cr', 'Co', 'V']
target = 0.058798885761114826 # target binding energy (Hartree) (1.6 eV)
q = 2 # Tsallis q param
num_epochs = 20 # number of epochs
maxit = 15 # max num iterations per epoch
...
def generic_inputs_init(adsorbate):
    ...
    f"electronic-minimize energyDiffThreshold 1e-7 nIterations 1000 nEnergyDiff 5\n"
    f"ionic-minimize energyDiffThreshold 1e-5 nIterations 1000 nEnergyDiff 3\n"
    f"\n"
...
def generic_inputs(adsorbate):
    ...
    f"symmetries none\n"
    f"# Relaxation commands\n"
    f"electronic-minimize energyDiffThreshold 1e-5 nIterations 1000 nEnergyDiff 5\n"
    f"\n"
    f"dump-name {adsorbate}.$VAR\n"
    f"dump End Lattice IonicPositions State MixGrad Ecomponents\n"
    f"\n")
...
lattice_constants = np.array([7.88, 8.26, 8.41, 8.01])
# specify spatial structure (here we want rock salt)
lattice_constant = np.mean(lattice_constants)  # average lattice constant of all involved RSO's
...
eta = 0.05 # Set entropy constant eta to small value

w_full = np.zeros((mix_count, num_species))
for i in range(mix_count):
    w_full[i,:] = 1/num_species + 0.05 * np.random.uniform(-1,1,num_species)
